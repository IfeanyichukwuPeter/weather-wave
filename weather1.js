...
function applyTheme() {
  ...
  // FORCE SEARCH DROPDOWN THEME UPDATE
  ...
  if (searchInput) {
    searchInput.blur(); // Reset focus to apply new theme
  }
}
...
async function updateWeatherDisplay(data, lat, lon, cityName) {
  ...
  document.getElementById("date").textContent = new Date().toLocaleDateString("en-NG", {
  ...
  // MAIN DEGREE WITH SNOWFLAKE FOR FREEZING TEMPS (but not when it's snowing)
  ...
  // Only show snowflake if freezing AND NOT already snowing (weather code 71)
  const isFreezing = data.current.temperature_2m <= 0;
  const hasSnowIcon = [71, 73, 75, 77, 85, 86].includes(data.current.weather_code);
  ...
  document.getElementById("feels-like").querySelector("h2").textContent = Math.round(feelsTemp) + "°";
  document.getElementById("humidity").querySelector("h2").textContent = data.current.relative_humidity_2m + "%";
  
  let windSpeed = data.current.wind_speed_10m;
  if (windUnit === "mph") windSpeed *= 0.621371;
  document.getElementById("wind").querySelector("h2").innerHTML = Math.round(windSpeed) + ` <span>${windUnit === "mph" ? "mph" : "km/h"}</span>`;
  document.getElementById("precipitation").querySelector("h2").textContent = data.current.precipitation.toFixed(1) + " in";
  ...
  updateMainCard(data.current.weather_code, data.current.is_day);
}
...
function updateMainCard(weatherCode, isDay) {
  ...
  const weatherThemes = {
    0: { day: "linear-gradient(135deg, #87CEEB, #98D8E8)", night: "linear-gradient(135deg, #1e3c72, #2a5298)" },
    1: { day: "linear-gradient(135deg, #F9D71C, #E9B10A)", night: "linear-gradient(135deg, #4B5EAA, #2A3F88)" },
    3: { day: "linear-gradient(135deg, #B0B0B0, #D0D0D0)", night: "linear-gradient(135deg, #404040, #606060)" },
    61: { color: "linear-gradient(135deg, #667eea 0%, #764ba2 100%)" },
    71: { color: "linear-gradient(135deg, #A8E6CF, #88D8A3)" }
  };
  const themeData = weatherThemes[weatherCode] || { day: "linear-gradient(135deg, #87CEEB, #98D8E8)" };
  mainCard.style.background = isDay ? themeData.day || themeData.color : themeData.night || themeData.color;
}
...
function updateHourlyForecast(hourly) {
  document.querySelectorAll(".hour-item").forEach((item, i) => {
    if (i < 10 && item) {
      ...
      item.querySelector(".hour-time").textContent = time.toLocaleTimeString([], { hour: "numeric", hour12: false });
      ...
      item.querySelector(".hour-icon").textContent = getWeatherIcon(hourly.weather_code[i]);
    }
  });
}
...
function updateDailyForecast(daily) {
  document.querySelectorAll(".day-item").forEach((item, i) => {
    if (i < 7 && item) {
      item.querySelector(".day-name").textContent = new Date(daily.time[i]).toLocaleDateString("en-US", { weekday: "short" });
      let maxTemp = daily.temperature_2m_max[i], minTemp = daily.temperature_2m_min[i];
      if (tempUnit === "fahrenheit") {
        maxTemp = (maxTemp * 9) / 5 + 32;
        minTemp = (minTemp * 9) / 5 + 32;
      }
      item.querySelector(".day-temps").textContent = Math.round(maxTemp) + "° / " + Math.round(minTemp) + "°";
      item.querySelector(".day-icon").textContent = getWeatherIcon(daily.weather_code[i]);
    }
  });
}
...
async function getCityName(lat, lon) {
  try {
    const response = await fetch(`https://geocoding-api.open-meteo.com/v1/search?latitude=${lat}&longitude=${lon}&language=en&format=json`);
    const data = await response.json();
    return data.results?.[0] ? `${data.results[0].name}, ${data.results[0].country}` : "Your Location";
  } catch {
...
document.querySelectorAll(".tab").forEach(tab => {
  tab.addEventListener("click", () => {
    ...
    document.querySelector(".hourly-forecast").style.display = tab.textContent === "Hourly forecast" ? "grid" : "none";
    document.querySelector(".daily-forecast").style.display = tab.textContent === "Daily forecast" ? "grid" : "none";
  });
});
...
function showSuggestions(suggestions) {
  ...
  suggestions.forEach(city => {
    ...
    div.innerHTML = `<strong>${city.name}</strong>, ${city.country}<br><small>${city.admin1 || ""}</small>`;
...
async function selectCity(city) {
  ...
  await updateWeatherDisplay(weather, city.latitude, city.longitude, displayName);
  hideLoader();
}
...
searchInput.addEventListener("focus", () => {
  if (searchInput.value && searchSuggestions.length) showSuggestions(searchSuggestions);
});
...
// ALERTS & CLOCK
const ALERTS = { 55: "🌨️ Heavy Freezing Rain", 65: "🌧️ Heavy Rain", 71: "❄️ Light Snow", 95: "⛈️ Thunderstorm" };
...
document.addEventListener("click", (e) => {
  ...
  if (e.target.matches('#settingsPanel [data-unit], #settingsPanel [data-theme]')) {
    e.stopPropagation();
    const btn = e.target;
    const groupSpan = btn.closest(".option-group")?.querySelector("span");
    const groupText = groupSpan?.textContent.trim();
